...
export default function DisplayContainer({fetchedData}){
    ...
    return (
        ...
        <Card className="restaurant" shadow="sm" key={index} isPressable onPress={() => console.log("item pressed")}>
          <CardBody className="overflow-visible p-0">
            ...
            <b>{item.name}</b>
            <p className="text-default-500">{item.address.street_addr}{', '}{item.address.city}{', '}{item.address.state}</p>
          </CardFooter>
        </Card>
      ))}
      </div>
